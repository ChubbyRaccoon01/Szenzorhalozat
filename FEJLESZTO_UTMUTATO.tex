...
\subsection{3. Program.cs -- Interaktív Alkalmazás}

\textbf{Fájl:} \texttt{Szenzorhalozat.Console/Program.cs}
...
\textbf{Főbb LINQ Módszerek:}
\begin{itemize}
    \item \texttt{OfType<T>()}: Típusspecifikus szűrés
    \item \texttt{Select()}: Szenzor string reprezentációra konvertálás
    \item \texttt{ToList()}: Eredmények gyűjteménnyé alakítása
    \item \texttt{?? Enumerable.Empty()}: Null-kezelés biztonságosan
\end{itemize}

\subsection{4. AdatgyujtoAllomas -- Adat Gyűjtés}

\textbf{Fájl:} \texttt{Szenzorhalozat.Core/AdatgyujtoAllomas.cs}
...
\subsection{5. Database -- Adatbáziskezelés}
...
\subsection{3. Hibakeresés és Futtatás}
...
\textbf{Alkalmazás futása után:}
\begin{itemize}
    \item 6 szenzor automatikus hozzáadása
    \item 5 mérési ciklus lebonyolítása
    \item Interaktív menü megjelenítése:
    \begin{lstlisting}[language=bash]
=== Main Menu ===
1. List Sensors
2. Export JSON
...
\subsection{4. Adatbázis Vizsgálata}

Az \texttt{Meres.db} fájl egy bináris LiteDB fájl. Lekérdezéshez:
\begin{itemize}
    \item Használd a \texttt{Database} osztály metódusait (\texttt{GetAllMeresiAdatok()})
    \item Vagy a LiteDB Studio-t bináris megjelenítéshez
    \item Vagy az alkalmazás menüjét (``List Database Content'')
\end{itemize}
